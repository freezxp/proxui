...
| Template build | `POST /nodes/{n}/storage/{s}/download-url` (`content=import`, optional `checksum`/`checksum-algorithm`) → `POST /nodes/{n}/qemu` → `POST …/config` with `scsi0=<storage>:0,import-from=<storage>:import/<file>` and the cloud-init drive → `POST …/{vmid}/template`. Needs `Datastore.AllocateTemplate`, `Sys.AccessNetwork`, `VM.Config.CDROM` and `VM.Config.HWType` (the last for `serial0`/`vga`/`scsihw`, which `POST /nodes/{n}/qemu` refuses with a bare 403 naming no privilege) — reported apart from the provisioning privileges |
| Template prep | After the disk is imported and before it is sealed, the node runs `virt-customize` over the portal's own SSH channel: installs qemu-guest-agent, truncates `/etc/machine-id`, deletes the SSH host keys and cloud-init state. Two fixed commands, no shell, the boundary ADR 0007 set. Needs `libguestfs-tools` on the node; without it the template is built unprepared and says so |
