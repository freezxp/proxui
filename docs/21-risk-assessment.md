...
| T2 | Proxmox API changes across 8.x/9.x releases | M | M | Token-authed stable `/api2/json` endpoints only; version captured at TestConnection; nightly lab conformance run | Lab run fails → version-gated code paths in connector only |
...
- PVE 8.x/9.x are both in scope; the connector is verified against a live 9.2 cluster. PVE 9 renamed the guest-agent privilege (`VM.Monitor` -> `VM.GuestAgent.Audit`), which is the kind of drift T2 anticipates.
