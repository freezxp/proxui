...
- **Palette:** shared with [dnsprox](https://github.com/freezxp/dnsprox), so two tools run by the same team read as one product. A cool slate ground, a single teal accent (`#0b6e99` light, `#5fb3d6` dark), and signal colours dark enough to be read as text rather than seen as decoration.
- **Typography:** IBM Plex Sans and IBM Plex Mono, bundled through `@fontsource` rather than linked from a font CDN — a portal somebody runs on their own network should not need Google to render, and an air-gapped install would otherwise fall back. Latin subsets only. Mono carries identity and metadata: the portal's name, an address, a fingerprint, a section heading.
- **Theme:** light/dark/system toggle in the account menu, persisted per browser. Semantic tokens (`bg-surface`, `text-muted`, state colors) so both themes stay consistent, and dark overrides only the values — no component knows which theme it is in. State colors: running=green, stopped=gray, paused=amber, missing/stale=orange (striped badge), deleted=red, unknown=slate.
